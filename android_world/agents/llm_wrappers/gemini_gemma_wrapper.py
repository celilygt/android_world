    ...
    def get_usage_summary(self) -> str:
        ...
        for model_name, config in GEMINI_MODEL_CONFIGS.items():
            ...
            summary_lines.append(
                f"  Minute: {minute_requests}/{config['rpm_limit']} requests, {minute_tokens:,}/{config['tpm_limit']:,} tokens")
...
class GeminiGemmaWrapper(
    ...
    def _generate_content_batch(
        ...
        for i, content in enumerate(contents):
            ...
            llm_logger.info(
                f"   📝 Prompt ({len(text_content)} chars, ~{estimated_tokens_list[i]} tokens):\n"
                f"---PROMPT START---\n{text_content}\n---PROMPT END---"
            )
            if image_count > 0:
            ...
            while counter > 0:
                try:
                    ...
                    if self.is_safe(output):
                        llm_logger.info(
                            f"✅ SUCCESS (request {i + 1}, {call_duration:.1f}s):\n"
                            f"---RESPONSE START---\n{output.text}\n---RESPONSE END---"
                        )
                    ...
                    llm_logger.error(
                        f"❌ ERROR (request {i + 1}, attempt {self.max_retry - counter + 1}, {call_duration:.1f}s): {e}")
