...
qwen_logger = logging.getLogger('qwen_llm_calls')
qwen_logger.propagate = False
if not qwen_logger.handlers:
    qwen_logger.addHandler(logging.StreamHandler())
...
class QwenVLWrapper(base_wrapper.MultimodalLlmWrapper):
    ...
    def predict_mm(
        ...
        qwen_logger.info(f"--- QWEN VL Request to {selected_model} ---")
        qwen_logger.info(f"Image count: {len(images)}")
        qwen_logger.info(f"---PROMPT START---\n{text_prompt}\n---PROMPT END---")
        ...
        while counter > 0:
            try:
                ...
                if 'choices' in response_json and response_json['choices']:
                    ...
                    qwen_logger.info(f"--- QWEN VL Response ---")
                    qwen_logger.info(f"---RESPONSE START---\n{response_content}\n---RESPONSE END---")
